...
//
//  A from-scratch VIEWING TRANSFORMATION: builds the view matrix manually
//  out of the u (right), v (up) and n (backward) basis vectors instead of
//  calling glm::lookAt, so the math behind the viewing transform is explicit.
//  Toggle it on in main.cpp (key B) to see the same scene rendered through
//  this hand-built matrix instead of the Camera class above.
//
