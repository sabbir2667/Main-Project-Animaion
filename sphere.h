...
//
//  A reusable unit sphere (position + normal attributes), used for the
//  ceiling-fan hub, lamp bulbs/shades and pillow shapes.
//
...
public:
    ...
    // draw with the sphere's own stored material color
    void drawSphere(Shader& lightingShader, glm::mat4 model) const
    {
        drawSphere(lightingShader, model, this->ambient, this->diffuse, this->specular, this->shininess);
    }

    // draw with an explicit color override (so one Sphere object can be
    // reused for a lamp shade here and a fan hub there with different colors)
    void drawSphere(Shader& lightingShader, glm::mat4 model, glm::vec3 amb, glm::vec3 diff, glm::vec3 spec, float shiny) const
    {
        lightingShader.use();
